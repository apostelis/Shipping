...
(uuid_generate_v4(), 'Maersk Logistics GmbH', 'LOGISTICS_PROVIDER', 'PLATINUM', 5, 'ops@maersk-logistics.com', NOW(), NOW()),
(uuid_generate_v4(), 'Shanghai Electronics Corp', 'INDUSTRIAL', 'GOLD', 4, 'shipping@sh-electronics.cn', NOW(), NOW()),
(uuid_generate_v4(), 'Mediterranean Fresh Foods', 'RETAIL', 'SILVER', 3, 'logistics@medfresh.eu', NOW(), NOW());
...
(uuid_generate_v4(), 'BK-2026-0451',
 ...
 'Electronics', 240, 3360000, 'PENDING', NOW(), NOW() + interval '5 days', NOW(), NOW()),
...
(uuid_generate_v4(), 'BK-2026-0452',
 ...
 'Mixed Cargo', 180, 2520000, 'PENDING', NOW(), NOW() + interval '3 days', NOW(), NOW()),
...
(uuid_generate_v4(), 'BK-2026-0441',
 ...
 'Electronics', 180, 2520000, 'CONFIRMED', NOW() - interval '2 days', NOW() + interval '1 day', NOW(), NOW()),

(uuid_generate_v4(), 'BK-2026-0442',
 (SELECT id FROM customers WHERE name = 'Maersk Logistics GmbH'),
 (SELECT id FROM ports WHERE code = 'NLRTM'), (SELECT id FROM ports WHERE code = 'BRSSZ'),
 'Machinery', 200, 2800000, 'CONFIRMED', NOW() - interval '1 day', NOW() + interval '3 days', NOW(), NOW()),

-- In transit
(uuid_generate_v4(), 'BK-2026-0430',
 (SELECT id FROM customers WHERE name = 'Mediterranean Fresh Foods'),
 (SELECT id FROM ports WHERE code = 'ESVLC'), (SELECT id FROM ports WHERE code = 'USLAX'),
 'Wine & Olive Oil', 75, 1050000, 'IN_TRANSIT', NOW() - interval '10 days', NOW() - interval '8 days', NOW(), NOW()),

(uuid_generate_v4(), 'BK-2026-0431',
 (SELECT id FROM customers WHERE name = 'Shanghai Electronics Corp'),
 (SELECT id FROM ports WHERE code = 'CNSHA'), (SELECT id FROM ports WHERE code = 'DEHAM'),
 'Auto Parts', 280, 3920000, 'IN_TRANSIT', NOW() - interval '15 days', NOW() - interval '14 days', NOW(), NOW());
